...
func buildPortList(dstPortsArg string) []uint64 {
	if dstPortsArg == "default" {
		dstPortsArg = defaultPorts
	}

	commaSplitted := strings.Split(dstPortsArg, ",")
	var portList []uint64
	for _, cs := range commaSplitted {
		if strings.Index(cs, "-") != -1 {
			// handle port range defined by <from>-<to>
			rng := strings.Split(cs, "-")
			from, err := strconv.ParseUint(rng[0], 10, 16)
			if err != nil {
				log.Fatal("Error converting port <from> attribute to integer")
			}
			to, err := strconv.ParseUint(rng[1], 10, 16)
			if err != nil {
				log.Fatal("Error converting port <to> attribute to integer")
			}
			if from > to {
				log.Fatal("Port range must be <from>-<to> where <from> must be smaller than <to>")
			}
			...
			// handle single port
			port, err := strconv.ParseUint(cs, 10, 16)
			if err != nil {
...
func scanPort(dstIP net.IP, dstPortInt uint64) {
	...
	for {
		...
		if err != nil {
			...
			closedPortsChan <- dstPort.String()
			break
...
func printResults(openPortList *[]string, closedPortList *[]string) {
	...
	for _, op := range *openPortList {
		table.Append([]string{op, "TCP", "open"})
	}
	for _, cp := range *closedPortList {
		table.Append([]string{cp, "TCP", "closed"})
	}
